...
export const analyzeResume = async (text: string, industry?: string): Promise<AnalysisData> => {
  ...
  try {
    ...
    const sentenceCount = Math.max(1, text.split(/[.!?]+/).length - 1); // Ensure we don't divide by zero
    ...
    if (industry) {
      ...
      industryFitScore = Math.min(100, Math.max(40, Math.round((presentKeywords.length / Math.max(1, industryKeywords.length)) * 100)));
    }
    ...
    // Calculate overall scores with safeguards against invalid values
    const keywordsScore = Math.round(
      (hasEducation ? 1 : 0) * 0.2 * 100 + 
